...
from biformer.ops.decoder import BiDecoder
...
class Block(nn.Module):
    def __init__(self, dim, in_kv=False, drop_path=0., layer_scale_init_value=-1,
        ...
        if topk > 0:
            self.attn = BiLevelRoutingAttention(in_kv=in_kv, dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
                                        ...
                                        auto_pad=auto_pad)
            # self.attn_decoder = BiDecoder(dim=dim, num_heads=num_heads, n_win=n_win, qk_dim=qk_dim,
            #                             qk_scale=qk_scale, kv_per_win=kv_per_win, kv_downsample_ratio=kv_downsample_ratio,
            #                             kv_downsample_kernel=kv_downsample_kernel, kv_downsample_mode=kv_downsample_mode,
            #                             topk=topk, param_attention=param_attention, param_routing=param_routing,
            #                             diff_routing=diff_routing, soft_routing=soft_routing, side_dwconv=side_dwconv,
    ...
    def forward(self, x):
        ...
        if self.decoder==True:
            self.attn = self.attn_decoder
        if self.pre_norm:
            if self.use_layer_scale:
                x = x + self.drop_path(self.gamma1 * self.attn(self.norm1(x))[0]) # (N, H, W, C)
...
class BiFormer(nn.Module):
    def __init__(self, in_kv=False, depth=[3, 4, 8, 3], in_chans=3, num_classes=1000, embed_dim=[64, 128, 320, 512],
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[Block(dim=embed_dim[i], in_kv=in_kv, drop_path=dp_rates[cur + j], 
...
if __name__ == "__main__":
    # test
    model = biformer_base(pretrained=True)
